...
resource "aws_ssoadmin_account_assignment" "digital_transformation_office_ai_staging" {
  for_each = { for perm in local.digital_transformation_office_ai_staging_permission_sets : "${perm.group.display_name}-${perm.permission_set.name}" => perm }
  ...
  target_id   = local.digital_transformation_office_ai_staging_account_id
  target_type = "AWS_ACCOUNT"
}
